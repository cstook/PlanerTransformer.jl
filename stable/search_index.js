...

{
    ...
    "title": "Introduction",
    "category": "section",
    "text": "This module is based on the design procedure described in Designing Planar Magnetics.Load the module.using PlanerTransformerDefine the PCB.trace_edge_gap = 0.16e-3\ntrace_trace_gap = 0.13e-3\nouter_copper_thickness = copper_weight_to_meters(1.0)\ninner_copper_thickness = copper_weight_to_meters(1.0)\nnumber_of_layers = 4\npcb = PCB_Specification(trace_edge_gap,\n                        trace_trace_gap,\n                        outer_copper_thickness,\n                        inner_copper_thickness,\n                        number_of_layers)Define core geometry and type of ferrite, or use data from core_geometry_dict and ferrite_dict.e_core = core_geometry_dict[\"e14\"]plate = core_geometry_dict[\"plt14\"]material = ferrite_dict[\"3f4\"]Define the windings.  In this case the primary will be 3 turns, 3 on each outer layer, both layers in parallel.  The secondary will be 4 turns, 2 turns on each inner layer, both layers in series.layer1 = winding_layer(pcb,true,e_core,3)\nlayer2 = winding_layer(pcb,false,e_core,2)\nlayer3 = winding_layer(pcb,false,e_core,2)\nlayer4 = winding_layer(pcb,true,e_core,3)\nprimary = Winding(pcb,[layer1,layer4],false)\nsecondary = Winding(pcb,[layer2,layer3],true)Define the magnetics.  Effective areas for cores in the magnetic circuit must match.magnetics = Magnetics(material, [e_core, plate])Define the transformer.  The first element in the winding array should be the input to the transformer.transformer = Transformer(magnetics, [primary, secondary])Now that the transformer is defined, we can estimate some circuit parameters. First pick an operating frequency, and a maximum specific power loss for the core.frequency = 1e6 # Hz\nspl_max = 450e3 # W/m^3Corresponding maximum peak to peak flux density.flux_density_max_pp = flux_density(transformer, spl_max, frequency) # TeslaMaximum volt seconds on each winding.volt_seconds_max = volt_seconds(transformer, flux_density_max_pp)Power Dissipation with 0.5Vpp 50% duty cycle square wave on the primary and 1App load on the secondary.primary_voltage_pp = 0.5\noutput_current_pp = 1.0\ntpd =TransformerPowerDissipation(transformer,\n              [primary_voltage_pp, output_current_pp], frequency)\ntotal_power = tpd.total_power # WParameters for a LTspice transformer model can also be computed.r1_r2 = winding_resistance(transformer)l1 = chan_inductor(transformer)r3 = equivalent_parallel_resistance(transformer, primary_voltage_pp, frequency)n1_n2 = turns(transformer)(Image: LTspice transformer model)"
},
